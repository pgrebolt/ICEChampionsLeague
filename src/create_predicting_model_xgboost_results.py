...
print(encoder_scores.inverse_transform(np.unique(Scores_training)))
print(matches_df[matches_df['Local']==0][matches_df['Visitant']==2])
...
#for match in range(int(0.2*matches_df.shape[0])+1, matches_df.shape[0]+1): # per cada partit disputat. Treiem els primers partits que no representen l'ELO real dels jugadors
    ...
    close_wins_visitant = frequencies_xr.sel(teammate=match_df['Jugador 1'], player = match_df['Jugador 2'])['CloseWinsPlayed'].values.item()
...
X_train, X_test, y_score_train, y_score_test = (
    train_test_split(Stats_training_stand, Scores_training, test_size=0.2))

# Second split: from the training set, carve out a validation set
X_train, X_val, y_score_train, y_score_val = train_test_split(
    X_train, y_score_train, test_size=0.1, random_state=42, stratify=y_score_train
)
...
if len(np.unique(Scores_training)) == 2:
    negatives = (Scores_training == 0).sum()
    positives = (Scores_training == 1).sum()
...
search.fit(X_train, y_score_train)

# 6) Evaluate on test set
best = search.best_estimator_
y_pred = best.predict(X_test)
print("Best params:", search.best_params_)
print(classification_report(y_score_test, y_pred))
ConfusionMatrixDisplay.from_predictions(y_score_test, y_pred)
plt.savefig('../results/Confusion_Matrix_Test_scores.png', dpi=300, bbox_inches='tight')
plt.clf()
...
plt.savefig('../results/shap_summary_scores.png', dpi=300, bbox_inches='tight')
plt.clf()
